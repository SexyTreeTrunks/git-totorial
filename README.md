...
> **주의사항**  
이번과제는 위의 '자바수업때 만든 코드들을 github에 업로드해보기' 과제를 한다음에 해주세요!

1. 자바수업코드들이 있는 프로젝트에서 git bash를 실행합니다
2. 마스터 브랜치에서 새로운 브랜치를 만듭니다(브랜치이름은 levelup으로 해주세요!)
3. 새로만든 브랜치로 이동합니다
4. 새로만든 브랜치에서 5개의 커밋을 만들어보세요(변경할 내용은 아무렇게나 하셔도 상관없습니다. 아무거나 수정하시고 커밋해보세요!)
> #### 주의사항 : 이번커밋은 원격저장소에 업로드하지마세요!

5. 이제 커밋을 취소해봅시다
	- `git log --oneline` 으로 커밋목록을 확인하고 캡쳐하세요! 
		+ 캡쳐한 이미지 이름을 '커밋로그 - 커밋취소전' 으로 해주세요!
	- `git reset --hard` 명령어를 이용하여 **현재상태에서 두단계 전의 커밋**으로 돌아가세요
	- `git status` 명령어로 현재 상황을 확인해봅시다
	- `git log --oneline` 으로 커밋목록을 확인하고 캡쳐하세요!
		+ 캡쳐한 이미지 이름을 '커밋로그 - 커밋취소후' 으로 해주세요!
	- 캡쳐한 '커밋로그 - 커밋취소전' 이미지와 '커밋로그 - 커밋취소후' 이미지를 비교해보세용
